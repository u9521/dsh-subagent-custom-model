export function classifySkillSource(skill) {
    ...
    const metaType = typeof skill.metadata?.type === 'string'
        ? skill.metadata.type.toLowerCase()
        : typeof skill.metadata?.scope === 'string'
            ? skill.metadata.scope.toLowerCase()
            : undefined;
    if (metaType === 'user' ||
        metaType === 'user-dsh' ||
...
export function compareSkills(a, b) {
...
export async function getAvailableSkills(ctx, sessionId) {
    const map = new Map();
...
export async function getSkillDetail(ctx, name, sessionId) {
    let targetCwd = undefined;
    if (sessionId) {
        try {
            const sessionsService = ctx.get('sessions');
