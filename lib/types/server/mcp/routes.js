import { loadMcpStore, saveMcpStore } from './storage.js';
import { testMcpConnection } from './tester/index.js';
import { readRequestBody } from '../common/http.js';
export function registerMcpRoutes(webServer, getMcpStore, setMcpStore, mcpManager) {
        ...
        handler: async (req, res) => {
            ...
            if (req.method === 'POST') {
                try {
                    ...
                    const transport = rawTransport === 'stdio'
                        ? 'stdio'
                    ...
                    const now = Date.now();
                    const toolCallTimeoutMs = typeof incoming.toolCallTimeoutMs === 'number' &&
                        incoming.toolCallTimeoutMs > 0
                        ? Math.floor(incoming.toolCallTimeoutMs)
                        : undefined;
                    const failOnStartupError = typeof incoming.failOnStartupError === 'boolean'
                        ? incoming.failOnStartupError
                        : undefined;
                    let reconnect = undefined;
                    if (incoming.reconnect && typeof incoming.reconnect === 'object') {
                        reconnect = {
                            enabled: typeof incoming.reconnect.enabled === 'boolean'
                                ? incoming.reconnect.enabled
                                : undefined,
                            initialDelayMs: typeof incoming.reconnect.initialDelayMs === 'number' &&
                                incoming.reconnect.initialDelayMs >= 0
                                ? Math.floor(incoming.reconnect.initialDelayMs)
                                : undefined,
                            maxDelayMs: typeof incoming.reconnect.maxDelayMs === 'number' &&
                                incoming.reconnect.maxDelayMs >= 0
                                ? Math.floor(incoming.reconnect.maxDelayMs)
                                : undefined,
                            maxAttempts: typeof incoming.reconnect.maxAttempts === 'number' &&
                    ...
                    const disabledTools = Array.isArray(incoming.disabledTools)
                        ? Array.from(new Set(incoming.disabledTools
                    ...
                    const serverConfig = {
                        ...
                        args: Array.isArray(incoming.args)
                            ...
                            : [],
                        env: incoming.env && typeof incoming.env === 'object'
                            ? incoming.env
                            : undefined,
                        cwd: incoming.cwd?.trim() || undefined,
                        url: incoming.url?.trim() || undefined,
                        headers: incoming.headers && typeof incoming.headers === 'object'
                            ? incoming.headers
                            : undefined,
                        ...
                        failOnStartupError,
                        reconnect,
                        disabledTools: disabledTools && disabledTools.length > 0
                            ? disabledTools
                            : undefined,
