 ...
 */
import { extractJsonRpcFromText, extractSupportedProtocolVersion, formatMcpError, } from './protocol.js';
import { startSseStream } from './sse.js';
/** Test an MCP server over Streamable HTTP or SSE transport */
export async function testHttpConnection(server) {
    ...
    try {
        ...
        if (initResult.json?.error &&
            ...
            const supportedVersion = extractSupportedProtocolVersion(initResult.json.error) ||
        ...
        if (initResult.json?.result && typeof initResult.json.result === 'object') {
            ...
            serverInfo = {
                name: typeof initResult.json.result.serverInfo?.name === 'string'
                    ? initResult.json.result.serverInfo.name
                    : undefined,
                version: typeof initResult.json.result.serverInfo?.version === 'string'
        ...
        if ((initResult.json?.error || !initResult.res.ok) &&
            (activeProtocolVersion === '2026-07-28' ||
                ...
                initResult.json?.error?.code === -32022)) {
            const statelessVersion = activeProtocolVersion === '2026-07-28' ||
                /2026-07-28/i.test(initResult.json?.error?.message || '') ||
                /2026-07-28/i.test(JSON.stringify(initResult.json?.error?.data || ''))
                ? '2026-07-28'
            ...
            try {
                ...
                if (Array.isArray(statelessTools)) {
                    const toolDetails = statelessTools
                        .map((t) => ({
