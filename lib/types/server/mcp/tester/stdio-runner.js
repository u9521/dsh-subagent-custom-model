...
import { createJsonRpcParser, extractSupportedProtocolVersion, formatMcpError, } from './protocol.js';
/** Test an MCP server over STDIO transport */
export async function testStdioConnection(server) {
    ...
    return new Promise((resolve) => {
        ...
        const parser = createJsonRpcParser((msg) => {
            ...
            if (msg.id === 1) {
                if (msg.error) {
                    const supportedVersion = extractSupportedProtocolVersion(msg.error) ||
                ...
                if (msg.result && typeof msg.result === 'object') {
                    ...
                    serverInfo = {
                        name: typeof msg.result.serverInfo?.name === 'string'
                            ? msg.result.serverInfo.name
                            : undefined,
                        version: typeof msg.result.serverInfo?.version === 'string'
