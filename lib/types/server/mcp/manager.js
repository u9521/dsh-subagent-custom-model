...
import { saveMcpStore } from './storage.js';
import { checkMcpCompatibility, } from './compatibility/index.js';
 ...
 */
export async function loadOfficialMcpClientPlugin() {
    if (officialPluginChecked)
        return cachedOfficialPlugin;
...
export class McpManager {
    ...
    isServerNeeded(serverId) {
        ...
        if (sessionSettingsStore) {
            if (sessionSettingsStore.default?.mcp?.mode === 'custom' &&
                sessionSettingsStore.default.mcp.enabledServerIds?.includes(serverId)) {
                return true;
            }
            if (sessionSettingsStore.sessions) {
                for (const sessionConfig of Object.values(sessionSettingsStore.sessions)) {
                    if (sessionConfig.mcp?.mode === 'custom' &&
        ...
        const syncPromise = (async () => {
            try {
                ...
                if (officialPlugin) {
                    ...
                    console.error('[session-settings] Official @deepseek-ai/dsh-mcp-client plugin not found in DSH environment.');
                }
            }
            catch (err) {
