...
import { IconRefreshOutline16, IconSearchOutline16, IconSkillOutline16, IconCloseOutline16, } from '@deepseek-ai/dsh-client-ui-primitives';
import { getLocalSessionSettingsStore, saveLocalSessionSettingsStore, } from './storage.js';
const e = React.createElement;
export function getSkillSourceMeta(skill, t) {
    if (!skill) {
        return {
...
export function SkillsSettingsTab({ api: _api, t, close: _close, }) {
    const localStore = getLocalSessionSettingsStore();
    const [skills, setSkills] = React.useState([]);
    const [defaultDisabledModelList, setDefaultDisabledModelList] = React.useState(localStore.default?.skills?.disabledModelSkills ||
    ...
    const enabledCount = React.useMemo(() => nonRuntimeSkills.filter((s) => !defaultDisabledModelSet.has(s.name))
        .length, [nonRuntimeSkills, defaultDisabledModelSet]);
            ...
            if (res.ok) {
                ...
                if (data?.ok) {
                    ...
                    if (data.defaultConfig?.skills) {
                        const mList = data.defaultConfig.skills.disabledModelSkills ||
    ...
    const handleToggleModelInvocable = (skillName) => {
        setSuccessMsg('');
        setError('');
        setDefaultDisabledModelList((prev) => prev.includes(skillName)
    ...
    const handleToggleUserInvocable = (skillName) => {
        setSuccessMsg('');
        setError('');
        setDefaultDisabledUserList((prev) => prev.includes(skillName)
            ? prev.filter((n) => n !== skillName)
            : [...prev, skillName]);
    };
    ...
    return e('div', { className: 'dsh-sam-page dsh-mcp-settings-page' }, 
    // Notice alert banners
    error ? e('div', { className: 'dsh-sam-notice error' }, error) : null, successMsg
        ? e('div', { className: 'dsh-sam-notice success' }, successMsg)
        : null, 
    // Page Header
    e('div', { className: 'dsh-mcp-header-card' }, e('div', { className: 'dsh-mcp-header-title-row' }, e('div', null, e('div', { style: { display: 'flex', alignItems: 'center', gap: '8px' } }, e(IconSkillOutline16, { size: 18 }), e('h2', { className: 'dsh-mcp-page-title' }, t('title') || '技能管理')), e('p', { className: 'dsh-mcp-page-desc' }, t('desc') ||
        '配置所有新会话默认生效的 Skill 启用/禁用规则。支持分别独立管控模型的 tool-skill 工具调用与用户的 /快捷指令。')), e('div', { className: 'dsh-mcp-header-actions' }, e('button', {
        type: 'button',
    ...
    // Content Body
    loading
        ? e('div', { className: 'dsh-mcp-loading-card' }, e('p', null, t('sessionSettings.skills.refreshing')))
        : e('div', { className: 'dsh-mcp-body-wrap' }, 
        // Toolbar
        e('div', { className: 'dsh-mcp-tools-toolbar' }, e('div', { className: 'dsh-mcp-tools-search-box' }, e('div', { className: 'dsh-mcp-search-wrap' }, e(IconSearchOutline16, {
            size: 14,
        ...
        // Stats Bar
        e('div', { className: 'dsh-mcp-tools-stats-bar' }, e('span', null, t('sessionSettings.skills.effectiveInfoDefault', {
            total: nonRuntimeSkills.length,
            enabled: enabledCount,
        })), search.trim()
            ? e('span', null, `匹配到 ${filteredSkills.length} / ${skills.length} 个技能`)
            : null), 
        // Skill list cards
        filteredSkills.length === 0
            ? e('div', { className: 'dsh-mcp-empty-card' }, e('p', { className: 'dsh-mcp-empty-text' }, t('sessionSettings.skills.noMatch')))
            : e('div', { className: 'dsh-session-skills-list' }, filteredSkills.map((skill) => {
