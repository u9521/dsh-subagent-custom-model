...
import { IconPlusOutline16, IconRefreshOutline16, IconEditOutline16, IconTrashOutline16, IconCloseOutline16, IconCodeOutline16, IconLinkOutline16, IconPlayOutline16, IconWarningOutline16, IconLoadingOutline16, IconChecklistOutline14, } from '@deepseek-ai/dsh-client-ui-primitives';
import { saveLocalMcpServers } from './storage.js';
...
export function McpServersSettingsTab({ api: _api, t, close: _close, }) {
    ...
    const [toolsDisabledSet, setToolsDisabledSet] = React.useState(new Set());
    const [toolsSearch, setToolsSearch] = React.useState('');
    const [toolsExpandedSchemas, setToolsExpandedSchemas] = React.useState(new Set());
    ...
    const handleOpenEdit = (server) => {
        setIsEditing(true);
        setShowAdvanced(Boolean(server.toolCallTimeoutMs ||
        ...
        setEnvEntries(server.env
            ? Object.entries(server.env).map(([key, value]) => ({ key, value }))
            : []);
        setHeaderEntries(server.headers
    ...
    const handleDelete = async (server) => {
        ...
        if (!window.confirm(confirmText))
            return;
        try {
            const res = await fetch(`/api/mcp-servers?id=${encodeURIComponent(server.id)}`, {
                method: 'DELETE',
            });
            if (res.ok) {
                const data = await res.json();
    ...
    const handleFormTest = async () => {
        ...
        for (const item of headerEntries) {
            if (item.key.trim())
                headerMap[item.key.trim()] = item.value;
        }
        const serverObj = {
            ...
            toolCallTimeoutMs: formServer.toolCallTimeoutMs && formServer.toolCallTimeoutMs > 0
        ...
        try {
            ...
            if (data.ok) {
                ...
                if (Array.isArray(data.toolDetails)) {
                    ...
                    setToolsList(data.tools.map((t) => typeof t === 'string' ? { name: t } : t));
                }
            }
            else {
    ...
    const handleSaveToolsModal = async () => {
        ...
        setToolsSaving(true);
        try {
            const targetServer = servers.find((s) => s.id === toolsTargetServer.id) || toolsTargetServer;
            const updatedServer = {
                ...targetServer,
                disabledTools: disabledArray,
            };
    ...
    const filteredTools = React.useMemo(() => {
        ...
        if (!q)
            return toolsList;
        return toolsList.filter((t) => t.name.toLowerCase().includes(q) ||
    ...
    const handleSaveForm = async () => {
        ...
        const payload = {
            ...
            toolCallTimeoutMs: formServer.toolCallTimeoutMs && formServer.toolCallTimeoutMs > 0
                ...
                : undefined,
            failOnStartupError: Boolean(formServer.failOnStartupError),
            disabledTools: formServer.disabledTools && formServer.disabledTools.length > 0
            ...
            reconnect: {
                ...
                initialDelayMs: formServer.reconnect?.initialDelayMs !== undefined &&
                    formServer.reconnect.initialDelayMs >= 0
                    ? Number(formServer.reconnect.initialDelayMs)
                    : undefined,
                maxDelayMs: formServer.reconnect?.maxDelayMs !== undefined &&
                    formServer.reconnect.maxDelayMs >= 0
                    ? Number(formServer.reconnect.maxDelayMs)
                    : undefined,
                maxAttempts: formServer.reconnect?.maxAttempts !== undefined &&
        ...
        try {
            ...
            for (const [key, val] of Object.entries(mcpDict)) {
                if (!val || typeof val !== 'object')
                    continue;
                const item = val;
                const transport = item.transport === 'stdio' || (item.command && !item.url)
                    ? 'stdio'
                ...
                const serverObj = {
                    ...
                    toolCallTimeoutMs: typeof item.toolCallTimeoutMs === 'number' &&
                        item.toolCallTimeoutMs > 0
                        ? item.toolCallTimeoutMs
                        : undefined,
                    failOnStartupError: typeof item.failOnStartupError === 'boolean'
                        ? item.failOnStartupError
                        : undefined,
                    reconnect: item.reconnect && typeof item.reconnect === 'object'
                        ? item.reconnect
                        : undefined,
                };
    ...
    return e('div', { className: 'dsh-mcp-settings-page' }, 
    // Header & Action Bar
    e('div', { className: 'dsh-mcp-header-card' }, e('div', { className: 'dsh-mcp-header-title-row' }, e('div', null, e('h2', { className: 'dsh-mcp-page-title' }, t('title')), e('p', { className: 'dsh-mcp-page-desc' }, t('desc'))), e('div', { className: 'dsh-mcp-header-actions' }, e('button', {
        type: 'button',
        className: 'dsh-sam-btn secondary',
        onClick: loadServers,
        title: t('actions.refresh'),
    }, e(IconRefreshOutline16, { size: 14 })), e('button', {
        type: 'button',
        className: 'dsh-sam-btn secondary',
        onClick: () => setImportOpen(true),
    }, t('actions.import')), e('button', {
        type: 'button',
        className: 'dsh-sam-btn primary',
        onClick: handleOpenAdd,
    }, e(IconPlusOutline16, { size: 14, className: 'dsh-btn-icon-left' }), t('actions.add'))))), 
    // Notifications
    successMsg
    ...
    loading
        ...
        : servers.length === 0
            ? e('div', { className: 'dsh-mcp-empty-card' }, e('div', { className: 'dsh-mcp-empty-icon' }, e(IconCodeOutline16, { size: 28 })), e('div', { className: 'dsh-mcp-empty-text' }, t('table.empty')), e('button', {
                type: 'button',
                className: 'dsh-sam-btn primary',
                onClick: handleOpenAdd,
            }, t('actions.add')))
            : e('div', { className: 'dsh-mcp-server-list' }, servers.map((server) => {
                ...
                const protoClass = server.transport === 'stdio'
                    ...
                    : server.detectedTransport === 'sse'
                        ...
                        : server.detectedTransport === 'streamable-http'
                            ? 'streamable-http'
                            : 'streamable-http-or-sse';
                return e('div', { key: server.id, className: 'dsh-mcp-server-card' }, 
                // Card Top Row
                e('div', { className: 'dsh-mcp-card-top' }, e('div', { className: 'dsh-mcp-card-identity' }, e('div', { className: 'dsh-mcp-transport-icon' }, server.transport === 'stdio'
                    ? e(IconCodeOutline16, { size: 16 })
                    : e(IconLinkOutline16, { size: 16 })), e('div', { className: 'dsh-mcp-title-wrap' }, e('span', { className: 'dsh-mcp-card-name' }, server.name), e('span', { className: 'dsh-mcp-card-id' }, server.id))), e('div', { className: 'dsh-mcp-badges' }, e('span', { className: `dsh-mcp-proto-badge ${protoClass}` }, protoLabel), server.serverInfo?.version
                    ? e('span', {
                    ...
                    : server.serverInfo?.protocolVersion
                        ...
                        : null, server.toolCallTimeoutMs
                    ? e('span', { className: 'dsh-mcp-proto-badge timeout' }, `${server.toolCallTimeoutMs / 1000}s 超时`)
                    : null, server.disabledTools && server.disabledTools.length > 0
                    ? e('span', { className: 'dsh-mcp-proto-badge disabled-tools' }, t('toolsModal.disabledBadge', {
                        count: server.disabledTools.length,
                    }))
                    : null, server.compatibility?.status ===
                    ...
                    : server.compatibility?.status === 'downgrade-supported'
                        ? e('span', {
                            ...
                            version: server.compatibility.negotiatedVersion ||
                                '2025-11-25',
                        }))
                        : null, server.enabledByDefault
                    ? e('span', { className: 'dsh-mcp-default-badge' }, t('table.enabledDefault'))
                    : null)), 
                // Description
                server.description
                ...
                e('div', { className: 'dsh-mcp-target-box' }, server.transport === 'stdio'
                    ? e('code', { className: 'dsh-mcp-code-preview' }, `${server.command || ''} ${(server.args || []).join(' ')}`)
                ...
                testResult
                    ? e('div', {
                        ...
                        : null, e('span', null, testResult.message))
                    : null, 
                // Card Footer Actions
                e('div', { className: 'dsh-mcp-card-footer' }, e('div', {
                    style: {
                        display: 'flex',
                        gap: '6px',
                        alignItems: 'center',
                    ...
                    : e(IconPlayOutline16, { size: 13 }), isTesting ? t('actions.testing') : t('actions.test')), e('button', {
                    type: 'button',
                    className: 'dsh-mcp-mini-btn',
                    onClick: () => handleOpenTools(server, 'card'),
                    title: t('actions.toolsList'),
                }, e(IconChecklistOutline14, { size: 13 }), t('actions.toolsList'), server.disabledTools && server.disabledTools.length > 0
                    ? e('span', { className: 'dsh-mcp-mini-badge danger' }, String(server.disabledTools.length))
                    : null)), e('div', { className: 'dsh-mcp-footer-right' }, e('button', {
                    type: 'button',
                    className: 'dsh-mcp-icon-btn',
                    onClick: () => handleOpenEdit(server),
                    title: t('actions.edit'),
                }, e(IconEditOutline16, { size: 14 })), e('button', {
                    type: 'button',
    ...
    formOpen
        ...
        }, e('div', { className: 'dsh-sam-modal-panel dsh-mcp-form-modal' }, 
        // Header
        e('div', { className: 'dsh-sam-header-row' }, e('h3', { className: 'dsh-sam-title' }, isEditing ? t('form.editTitle') : t('form.addTitle')), e('button', {
            type: 'button',
        ...
        formError ||
            ...
            ? e('div', { className: 'dsh-sam-notices-block' }, formError
                ...
                : null, formServer.compatibility?.canEnable === false ||
                formServer.compatibility?.status ===
                    'incompatible-2026-07-28'
                ? e('div', { className: 'dsh-sam-notice error' }, `⚠️ ${t('compatibility.incompatibleBadge')}: ${formServer.compatibility?.warning || t('compatibility.incompatibleDesc')}`)
                : formServer.compatibility?.status === 'downgrade-supported'
                    ? e('div', { className: 'dsh-sam-notice info' }, `ℹ️ ${t('compatibility.downgradedBadge', { version: formServer.compatibility.negotiatedVersion || '2025-11-25' })}: ${formServer.compatibility?.warning || t('compatibility.downgradedDesc')}`)
                    : null)
            : null, 
        // Form Content
        e('div', { className: 'dsh-mcp-form-body' }, 
        ...
        }, e('div', { className: 'dsh-mcp-switch-text' }, e('div', { className: 'dsh-mcp-switch-title' }, t('form.enabledByDefault')), e('div', { className: 'dsh-mcp-switch-desc' }, t('form.enabledByDefaultDesc'))), e('div', {
            className: `dsh-mcp-switch-btn ${formServer.enabledByDefault ? 'active' : ''}`,
            'aria-hidden': 'true',
        }, e('span', { className: 'dsh-mcp-switch-thumb' }))), 
        // ID & Name Row
        e('div', { className: 'dsh-mcp-form-row' }, e('div', { className: 'dsh-sam-field-group flex-1' }, e('label', { className: 'dsh-sam-field-label' }, t('form.id')), e('input', {
            type: 'text',
            className: 'dsh-sam-select',
            placeholder: t('form.idPlaceholder'),
            disabled: isEditing,
            value: formServer.id || '',
            onChange: (evt) => setFormServer({ ...formServer, id: evt.target.value }),
        })), e('div', { className: 'dsh-sam-field-group flex-1' }, e('label', { className: 'dsh-sam-field-label' }, t('form.name')), e('input', {
            type: 'text',
            className: 'dsh-sam-select',
            placeholder: t('form.namePlaceholder'),
            value: formServer.name || '',
            onChange: (evt) => setFormServer({ ...formServer, name: evt.target.value }),
        }))), 
        // Description
        e('div', { className: 'dsh-sam-field-group' }, e('label', { className: 'dsh-sam-field-label' }, t('form.description')), e('input', {
            type: 'text',
            className: 'dsh-sam-select',
        ...
        // Transport Selector
        e('div', { className: 'dsh-sam-field-group' }, e('label', { className: 'dsh-sam-field-label' }, t('form.transport')), e('select', {
            className: 'dsh-sam-select',
        ...
        }, e('option', { value: 'stdio' }, t('form.transportStdio')), e('option', { value: 'streamable-http-or-sse' }, t('form.transportHttp')))), 
        // Transport specific fields
        formServer.transport === 'stdio'
            ? e(React.Fragment, null, 
            // Command
            e('div', { className: 'dsh-sam-field-group' }, e('label', { className: 'dsh-sam-field-label' }, t('form.command')), e('input', {
                type: 'text',
                className: 'dsh-sam-select',
            ...
            // Arguments
            e('div', { className: 'dsh-sam-field-group' }, e('label', { className: 'dsh-sam-field-label' }, t('form.args')), e('textarea', {
            ...
            // CWD
            e('div', { className: 'dsh-sam-field-group' }, e('label', { className: 'dsh-sam-field-label' }, t('form.cwd')), e('input', {
                type: 'text',
                className: 'dsh-sam-select',
            ...
            // ENV entries
            e('div', { className: 'dsh-sam-field-group' }, e('label', { className: 'dsh-sam-field-label' }, t('form.env')), envEntries.length > 0
                ? e('div', { className: 'dsh-mcp-kv-list' }, envEntries.map((item, idx) => e('div', { key: idx, className: 'dsh-mcp-kv-row' }, e('input', {
                    type: 'text',
                    className: 'dsh-sam-select',
                ...
                onClick: () => setEnvEntries([
                    ...envEntries,
                    { key: '', value: '' },
                ]),
            }, e(IconPlusOutline16, { size: 14 }), t('form.addEnv'))))
            : e(React.Fragment, null, 
            // URL
            e('div', { className: 'dsh-sam-field-group' }, e('label', { className: 'dsh-sam-field-label' }, t('form.url')), e('input', {
                type: 'text',
                className: 'dsh-sam-select',
            ...
            // Headers
            e('div', { className: 'dsh-sam-field-group' }, e('label', { className: 'dsh-sam-field-label' }, t('form.headers')), headerEntries.length > 0
                ? e('div', { className: 'dsh-mcp-kv-list' }, headerEntries.map((item, idx) => e('div', { key: idx, className: 'dsh-mcp-kv-row' }, e('input', {
                    type: 'text',
                    className: 'dsh-sam-select',
                ...
                type: 'button',
                className: 'dsh-mcp-add-btn',
                onClick: () => setHeaderEntries([
                    ...headerEntries,
                    { key: '', value: '' },
                ]),
            }, e(IconPlusOutline16, { size: 14 }), t('form.addHeader')))), 
        // Advanced & Runtime Settings (Collapsible Box)
        e('div', { className: 'dsh-mcp-advanced-box' }, e('div', {
            className: 'dsh-mcp-advanced-header',
            role: 'button',
            tabIndex: 0,
            onClick: () => setShowAdvanced((prev) => !prev),
        }, e('div', { className: 'dsh-mcp-advanced-title-wrap' }, e('span', { className: 'dsh-mcp-advanced-title' }, t('form.advancedTitle')), e('span', { className: 'dsh-mcp-advanced-badge' }, showAdvanced ? '收起' : '展开配置'))), showAdvanced
            ? e('div', { className: 'dsh-mcp-advanced-content' }, 
            // 1. Tool Call Timeout (ms)
            e('div', { className: 'dsh-sam-field-group' }, e('label', { className: 'dsh-sam-field-label' }, t('form.toolCallTimeoutMs')), e('input', {
                type: 'number',
                min: 1000,
                step: 1000,
                className: 'dsh-sam-select',
                placeholder: t('form.toolCallTimeoutMsPlaceholder'),
                value: formServer.toolCallTimeoutMs !== undefined
            ...
            }), e('span', { className: 'dsh-mcp-field-hint' }, t('form.toolCallTimeoutMsDesc'))), 
            ...
            }, e('div', { className: 'dsh-mcp-switch-text' }, e('div', { className: 'dsh-mcp-switch-title' }, t('form.failOnStartupError')), e('div', { className: 'dsh-mcp-switch-desc' }, t('form.failOnStartupErrorDesc'))), e('div', {
            ...
            }, e('div', { className: 'dsh-mcp-switch-text' }, e('div', { className: 'dsh-mcp-switch-title' }, t('form.reconnectEnabled')), e('div', { className: 'dsh-mcp-switch-desc' }, t('form.reconnectEnabledDesc'))), e('div', {
            ...
            formServer.reconnect?.enabled !== false
                ? e('div', { className: 'dsh-mcp-form-row-3' }, e('div', { className: 'dsh-sam-field-group' }, e('label', { className: 'dsh-sam-field-label' }, t('form.reconnectInitialDelayMs')), e('input', {
                    type: 'number',
                    min: 0,
                    step: 100,
                    className: 'dsh-sam-select',
                    placeholder: t('form.reconnectInitialDelayMsPlaceholder'),
                    value: formServer.reconnect?.initialDelayMs !==
                        undefined
                ...
                })), e('div', { className: 'dsh-sam-field-group' }, e('label', { className: 'dsh-sam-field-label' }, t('form.reconnectMaxDelayMs')), e('input', {
                    type: 'number',
                    min: 0,
                    step: 1000,
                    className: 'dsh-sam-select',
                    placeholder: t('form.reconnectMaxDelayMsPlaceholder'),
                    value: formServer.reconnect?.maxDelayMs !== undefined
                ...
                })), e('div', { className: 'dsh-sam-field-group' }, e('label', { className: 'dsh-sam-field-label' }, t('form.reconnectMaxAttempts')), e('input', {
                    type: 'number',
                    min: 0,
                    step: 1,
                    className: 'dsh-sam-select',
                    placeholder: t('form.reconnectMaxAttemptsPlaceholder'),
                    value: formServer.reconnect?.maxAttempts !==
                        undefined
        ...
        e('div', { className: 'dsh-sam-actions dsh-mcp-modal-footer' }, e('div', { className: 'dsh-mcp-modal-footer-left' }, e('button', {
            type: 'button',
            className: 'dsh-sam-btn secondary',
            ...
            : e(IconPlayOutline16, { size: 14 }), formTesting ? t('actions.testing') : t('actions.test')), e('button', {
            type: 'button',
            className: 'dsh-sam-btn secondary',
            disabled: formTesting || formSaving,
            onClick: handleFormOpenTools,
            title: t('actions.toolsList'),
        }, e(IconChecklistOutline14, { size: 14 }), t('actions.toolsList'), formServer.disabledTools &&
            formServer.disabledTools.length > 0
            ? e('span', { className: 'dsh-mcp-mini-badge danger' }, String(formServer.disabledTools.length))
            : null)), e('div', { className: 'dsh-mcp-modal-footer-right' }, e('button', {
            type: 'button',
            className: 'dsh-sam-btn secondary',
            disabled: formSaving || formTesting,
            onClick: () => setFormOpen(false),
        }, t('actions.cancel')), e('button', {
            type: 'button',
            className: 'dsh-sam-btn primary',
            disabled: formSaving || formTesting,
    ...
    toolsModalOpen && toolsTargetServer
        ...
        // Header
        e('div', { className: 'dsh-sam-header-row' }, e('div', null, e('h3', { className: 'dsh-sam-title' }, t('toolsModal.title')), e('div', { className: 'dsh-mcp-tools-header-meta' }, e('span', { className: 'dsh-mcp-card-name' }, toolsTargetServer.name || toolsTargetServer.id), e('span', { className: 'dsh-mcp-card-id' }, toolsTargetServer.id), e('span', {
            ...
            : toolsDetectedTransport === 'sse'
                ...
                : toolsDetectedTransport === 'streamable-http'
                    ? 'Streamable HTTP'
                    : 'HTTP / SSE'), toolsServerInfo?.version
            ? e('span', {
                className: 'dsh-mcp-proto-badge server-version',
            ...
            : toolsServerInfo?.protocolVersion
                ...
                : null, toolsTargetServer.compatibility?.status ===
            'incompatible-2026-07-28' ||
            toolsTargetServer.compatibility?.canEnable === false
            ? e('span', {
                className: 'dsh-mcp-proto-badge incompatible',
                title: toolsTargetServer.compatibility.warning ||
                    t('compatibility.incompatibleDesc'),
            }, t('compatibility.incompatibleBadge'))
            : toolsTargetServer.compatibility?.status ===
                ...
                ? e('span', {
                    ...
                    version: toolsTargetServer.compatibility
        ...
        e('div', { className: 'dsh-mcp-tools-toolbar' }, 
        // Search box
        e('div', { className: 'dsh-mcp-tools-search-box' }, e('input', {
            type: 'text',
            className: 'dsh-sam-select',
        ...
        e('div', { className: 'dsh-mcp-tools-toolbar-actions' }, e('button', {
        ...
        }, t('toolsModal.enableAll')), e('button', {
            type: 'button',
            className: 'dsh-sam-btn secondary',
            disabled: toolsLoading || toolsList.length === 0,
            onClick: () => handleToggleAllTools(false),
        }, t('toolsModal.disableAll')), e('button', {
            type: 'button',
            className: 'dsh-sam-btn secondary',
            ...
            : e(IconRefreshOutline16, { size: 14 }), toolsLoading
            ? t('actions.toolsFetching')
            : t('toolsModal.retry')))), 
        // Stats summary bar
        !toolsLoading && toolsList.length > 0
            ? e('div', { className: 'dsh-mcp-tools-stats-bar' }, e('span', null, t('toolsModal.summary', {
                total: toolsList.length,
                enabled: toolsList.length - toolsDisabledSet.size,
                disabled: toolsDisabledSet.size,
            })), toolsSearch.trim()
                ? e('span', null, t('toolsModal.filteredSummary', {
        ...
        toolsLoading
            ? e('div', {
                ...
                size: 20,
                className: 'dsh-spin',
            }), e('span', { style: { marginLeft: 8 } }, t('toolsModal.fetching')))
            : toolsError
                ? e('div', {
                ...
                : toolsList.length === 0
                    ...
                    : filteredTools.length === 0
                        ...
                        : e('div', { className: 'dsh-mcp-tools-list' }, filteredTools.map((tool) => {
                            ...
                            }, e('div', { className: 'dsh-mcp-tool-card-main' }, e('div', { className: 'dsh-mcp-tool-card-left' }, 
                            ...
                            // Tool info
                            e('div', { className: 'dsh-mcp-tool-info' }, e('div', {
                                className: 'dsh-mcp-tool-title-row',
                            }, e('span', { className: 'dsh-mcp-tool-name' }, tool.name), e('span', {
                                className: `dsh-mcp-tool-status-pill ${!isDisabled ? 'active' : 'disabled'}`,
                            }, !isDisabled
                                ? t('toolsModal.statusEnabled')
                                : t('toolsModal.statusDisabled'))), tool.description
                                ? e('p', { className: 'dsh-mcp-tool-desc' }, tool.description)
                                : null)), 
                            ...
                            isSchemaExpanded && hasSchema
                                ? (() => {
                                    ...
                                    return e('div', { className: 'dsh-mcp-tool-expanded-box' }, 
                                    ...
                                    }, t('toolsModal.viewList')), e('button', {
                                        type: 'button',
                                    ...
                                    mode === 'list'
                                        ? params.length > 0
                                            ? e('div', {
                                                ...
                                                : t('toolsModal.optional')), param.default !== undefined
                                                ? e('span', {
        ...
        e('div', {
            ...
            style: { marginTop: 14 },
        }, e('div', { className: 'dsh-mcp-modal-footer-left' }, toolsDisabledSet.size > 0
            ? e('span', { className: 'dsh-mcp-proto-badge disabled-tools' }, t('toolsModal.disabledBadge', {
    ...
    importOpen
        ? e('div', {
        ...
        }, e('div', { className: 'dsh-sam-modal-panel dsh-mcp-import-modal' }, e('div', { className: 'dsh-sam-header-row' }, e('h3', { className: 'dsh-sam-title' }, t('importModal.title')), e('button', {
            type: 'button',
            className: 'dsh-sam-close-btn',
            onClick: () => setImportOpen(false),
        }, e(IconCloseOutline16, { size: 16 }))), e('p', { className: 'dsh-sam-desc' }, t('importModal.desc')), importError
        ...
        }), e('div', { className: 'dsh-sam-actions' }, e('button', {
            type: 'button',
            className: 'dsh-sam-btn primary',
            ...
            : t('importModal.confirm')), e('button', {
            type: 'button',
            className: 'dsh-sam-btn secondary',
            onClick: () => setImportOpen(false),
        }, t('actions.cancel')))))
    ...
    saveConfirm?.open
        ...
        }, e('div', { className: 'dsh-sam-modal-panel dsh-mcp-confirm-modal' }, e('div', { className: 'dsh-sam-header-row' }, e('div', {
            style: {
                display: 'flex',
                alignItems: 'center',
                gap: '8px',
            },
        }, e(IconWarningOutline16, { size: 18 }), e('h3', { className: 'dsh-sam-title' }, t('saveConfirmModal.title'))), e('button', {
            type: 'button',
            className: 'dsh-sam-close-btn',
            onClick: () => setSaveConfirm(null),
        }, e(IconCloseOutline16, { size: 16 }))), e('p', { className: 'dsh-mcp-confirm-msg' }, t('saveConfirmModal.message')), e('div', { className: 'dsh-mcp-confirm-detail' }, saveConfirm.message), e('p', { className: 'dsh-mcp-confirm-prompt' }, t('saveConfirmModal.prompt')), e('div', { className: 'dsh-sam-actions' }, e('button', {
            type: 'button',
            className: 'dsh-sam-btn secondary',
            onClick: () => setSaveConfirm(null),
        }, t('saveConfirmModal.cancel')), e('button', {
            type: 'button',
            className: 'dsh-sam-btn primary',
