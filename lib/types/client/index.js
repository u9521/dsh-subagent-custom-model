...
import { LOCALE_NS } from './types.js';
import { SessionSettingsViewPage } from './SessionSettingsViewPage.js';
import { McpServersSettingsTab } from './McpServersSettingsTab.js';
import { SkillsSettingsTab } from './SkillsSettingsTab.js';
...
export function apply(ctx) {
    ...
    ctx.slots.inject('settings.section', () => ctx.slots.register({
        ...
        const t = React.useCallback((key, vars) => {
            ...
            if (res && res !== `skillsSettings.${cleanKey}`)
                return res;
            res = translator(`sessionSettings.skills.${cleanKey}`, vars);
            if (res && res !== `sessionSettings.skills.${cleanKey}`)
                return res;
            res = translator(`skillsSettings.${key}`, vars);
            if (res && res !== `skillsSettings.${key}`)
                return res;
            res = translator(`sessionSettings.${key}`, vars);
            if (res && res !== `sessionSettings.${key}`)
                return res;
