...
import {
  ...
  formatMcpError,
} from '../tester/protocol.ts'

export type CompatibilityStatus =
  'compatible' | 'downgrade-supported' | 'incompatible-2026-07-28' | 'unknown'
 ...
 */
export async function checkMcpCompatibility(
  server: Partial<GlobalMcpServerConfig>,
  discoveredInfo?: McpServerInfo,
  signal?: AbortSignal,
  ...
  if (server.transport === 'stdio') {
    const protocolVersion =
      discoveredInfo?.protocolVersion || server.serverInfo?.protocolVersion
    if (protocolVersion === '2026-07-28') {
      return {
        ...
        warning:
          '当前官方 @deepseek-ai/dsh-mcp-client 客户端暂未适配 2026-07-28 无状态协议，禁止直接启用。',
  ...
  try {
    ...
    const isStateless2026Feasible =
      (statelessRes.ok || statelessRes.status === 202) &&
    ...
    if (
      (legacyRes.ok || legacyRes.status === 202) &&
    ...
    if (legacyJson?.error) {
      ...
      const legacyCandidate = supportedList.find(
        (v) =>
          v === '2025-11-25' ||
          v === '2024-11-05' ||
          v.startsWith('2024-') ||
    ...
    if (isStateless2026Feasible) {
      return {
        ...
        message:
          '检测到 2026-07-28 无状态协议，经测试该服务端无法向下降级，官方客户端暂不支持',
        warning:
          '该 MCP 服务端为纯 2026-07-28 无状态协议（经测试不支持向下降级至 2024-11-05 / 2025-11-25），官方 @deepseek-ai/dsh-mcp-client 暂未支持，禁止启用以防异常。',
        error:
          '服务端仅支持 2026-07-28 无状态协议，不支持向下降级至旧版握手协议',
      }
    }
