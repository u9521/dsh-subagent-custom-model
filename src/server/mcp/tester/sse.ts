...
export async function discoverSseEndpoint(
  url: URL,
...
export function startSseStream(
      ...
      while (!abortSignal.aborted) {
        ...
        while (keepScanning) {
          ...
          if (actualEnd !== -1) {
            ...
            const endpointMatch =
              eventBlock.match(/event:\s*endpoint[\r\n]+data:\s*([^\r\n]+)/i) ||
  ...
  const timeoutEndpoint = new Promise<string>((res) =>
    setTimeout(() => res(parsedUrl.toString()), 2500),
  )
  const postEndpointPromise = Promise.race([endpointPromise, timeoutEndpoint])

  const waitForMessageWithId = (
    targetId: number,
    timeoutMs = 6000,
  ): Promise<any> => {
