...
export function createJsonRpcParser(onMessage: (msg: any) => void) {
  let buffer = ''
...
export function extractJsonRpcFromText(text: string): any {
...
export function formatMcpError(err: any): string {
  if (!err) return '未知错误'
...
export function extractSupportedProtocolVersion(error: any): string | null {
  ...
  if (
    ...
    const listMatch =
      message.match(/supported versions?:\s*([^\)\n]+)/i) ||
