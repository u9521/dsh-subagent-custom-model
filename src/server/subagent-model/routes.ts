...
import {
  normalizeSubagentModelConfig,
  resolveEffectiveSubagentModel,
  saveSessionSettingsStore as saveSessionSettingsStoreToDisk,
} from '../session/storage.ts'
import { readRequestBody } from '../common/http.ts'

export function registerLegacySubagentModelRoutes(
  webServer: any,
    ...
    handler: async (req: any, res: any) => {
      ...
      if (req.method === 'GET') {
        ...
        res.end(
          JSON.stringify({
            ...
            config: rawConfig,
            effectiveConfig: resolveEffectiveSubagentModel(
      ...
      if (req.method === 'POST') {
        try {
          ...
          const incomingConfig = normalizeSubagentModelConfig(
            parsed.config || parsed,
          )

          if (
            incomingConfig.mode === 'custom' &&
